...
//
//  Uses Firestore REST API — NO heavy SDK, just plain fetch().
//  The form summary appears INSTANTLY on submit.
//  Firestore save happens in the background silently.
//
//  SETUP (free, one-time, ~5 minutes):
//  1. Go to https://console.firebase.google.com
//  2. Add project → name it → Continue
//  3. Firestore Database → Create database → Start in test mode → Done
//  4. Go to Project Settings → copy your Project ID
//  5. Paste it below as FIREBASE_PROJECT_ID — that's the only thing to change!
// ============================================================

var FIREBASE_PROJECT_ID = 'YOUR_PROJECT_ID';  // <-- PASTE YOUR PROJECT ID HERE

var FIRESTORE_BASE = 'https://firestore.googleapis.com/v1/projects/' +
                     FIREBASE_PROJECT_ID + '/databases/(default)/documents/tmb_submissions';
...
function buildDoc(data) {
  ...
  return '' +
    '<div class="print-header">' +
      '<div class="print-logo-area"><div class="print-brand"><h1>Toyota Manila Bay</h1></div></div>' +
    ...
    '<div class="info-grid" style="margin-bottom:1.2rem">' +
      infoRow('Home Address',b.address) + infoRow('Length of Stay',b.los) +
      infoRow('Ownership',b.own) + infoRow("Mother's Maiden Name",b.mom) +
    '</div>' +
    ...
    '</div>' +
    '<div class="info-grid" style="margin-bottom:1.2rem">' +
      infoRow('Home Address',c.address) + infoRow('Length of Stay',c.los) +
      infoRow('Ownership',c.own) + infoRow("Mother's Maiden Name",c.mom) +
    '</div>' +
...
function fromFirestoreDoc(doc) {
  function parseFields(fields) {
    ...
    Object.keys(fields).forEach(function(key) {
      ...
      else if (f.mapValue     !== undefined) obj[key] = parseFields(f.mapValue.fields);
      else obj[key] = null;
    });
    return obj;
  }
  var data = parseFields(doc.fields);
  var parts = (doc.name || '').split('/');
  data.firestoreId = parts[parts.length - 1];
  return data;
}

// ============================================================
//  SUBMIT — INSTANT
//  Shows the output screen immediately, saves to cloud in background.
//  The user never waits for a network call.
// ============================================================
function submitForm() {
  ...
  .then(function() {
    // Saved successfully — user already sees the summary, nothing else needed
  })
  .catch(function(err) {
    console.error('Background save failed:', err);
    // Keep a local backup so data is never fully lost
    try {
      var fallback = JSON.parse(localStorage.getItem('tmb_fallback') || '[]');
      fallback.push(data);
      localStorage.setItem('tmb_fallback', JSON.stringify(fallback));
    } catch(e) {}
    showToast('Note: Cloud save failed. Check your Firebase Project ID in script.js.');
  });
}
...
function savePDF()  { setTimeout(function() { window.print(); }, 200); }

// ============================================================
//  ADMIN — loads all submissions from Firestore
// ============================================================
function renderAdmin() {
  var body = document.getElementById('admin-body');
  body.innerHTML = '<div class="empty-state">Loading submissions…</div>';

  fetch(FIRESTORE_BASE + '?pageSize=200', { method: 'GET' })
  .then(function(res) {
    if (!res.ok) throw new Error('HTTP ' + res.status);
    return res.json();
  })
  .then(function(json) {
    ...
    cachedSubmissions = docs.map(fromFirestoreDoc).sort(function(a, b) {
  ...
  .catch(function(err) {
    ...
    body.innerHTML =
      '<div class="empty-state" style="color:#CC0000;">' +
        'Could not load submissions.<br>' +
        'Make sure your Firebase Project ID is set correctly in script.js<br>' +
        'and that Firestore is enabled in test mode.' +
      '</div>';
  });
}
...
function deleteSubmission(i, firestoreId) {
  ...
  fetch(FIRESTORE_BASE + '/' + firestoreId, { method: 'DELETE' })
    .then(function(res) {
      if (!res.ok) throw new Error('HTTP ' + res.status);
